...
/**
 * A formulation of the travel salesperson problem
...
public class TSP implements Problem<List<Integer>> {
    ...
    public double cost(List<Integer> state){
        ...
        for(int i = 0, j=1; j<state.size(); i++, j++){
            totalDistance += MAP.distanceMatrix[state.get(i)][state.get(j)];
        }
        totalDistance += MAP.distanceMatrix[state.getLast()][state.getFirst()];
